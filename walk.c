...
/*   Updated: 2022/06/28 16:50:39 by tyanar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minlbx.h"

void	putchr(t_win *pnc, int keycode) // ekrana karaktere dogru yonde koyrdumka icin 
{
	if (keycode == 2)
...
void	print_score(t_win *pnc) // temrinale scor yazma icin 
{
	ft_printf("%d\n", pnc->chr->point);
}

void	move_p(t_win *pnc, int side, int x, int y) // karakter hareket 
{
...
int	walk(int keycode, void *param)// yürüme fonksiyonyu
{
	t_win	*pnc;
