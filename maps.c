...
/*   Updated: 2022/06/28 16:48:52 by tyanar           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minlbx.h"

char	*read_file(char	*path, t_map *map) // dosyaı okudugumuz ksım 
{
	char	*line;
	char	*ret;
	int		fd;
...
t_map	*read_map(char *path) // mapi struck yapısına koyuyoruz 
{
	t_map	*ret;
	char	*map;
...
void	put_element(char c, t_win *pnc, int x, int y) // gördügümüz char karakterine göre ekrana resim basıyoruz 
{
...
void	put_score(t_win *pnc) // ekrana scor basıyor BONUS;
{
	char	*score;
...
void	render_map(t_win *pnc)// mapi renderlıyor 
{
	int	i;
	int	j;
	int	x;
